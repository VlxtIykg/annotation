...
export async function POST(context) {
  ...
  const [formData, formDataError] = await handleTryCatch(
  ...
  if (fileContentError) {
    ...
    return new Response(
      JSON.stringify({
        status: 400,
        message:
          "Failed to read file content, please send an issue to the github! Don't forget to screenshot console via browser and/or read the issue.md! Thank you very much!",
  ...
  const [jsonContent, jsonError] = await handleTryCatch(
